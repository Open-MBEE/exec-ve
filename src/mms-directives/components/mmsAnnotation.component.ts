...
let MMSAnnotationComponent: angular.ve.ComponentOptions = {
    selector: 'mmsAnnotation',
    template: `
    <span class="mms-error" uib-popover-template="'annotationTemplate.html'" popover-popup-close-delay="500" ng-bind-html="displayContent.inlineContent" popover-placement="top-left" popover-title="{{$ctrl.displayContent.toolTipTitle}}"></span>
...
`,
    ...
    controller: class MMSAnnotationController implements angular.IComponentController {
       ...
       public copyToClipboard($event) {
            this.utilsSvc.copyToClipboard(this.$element.find('#tooltipElementId'), $event);
        };
...
mmsDirectives.component(MMSAnnotationComponent.selector, MMSAnnotationComponent);
