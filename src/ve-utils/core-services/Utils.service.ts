...
export class UtilsService {
    ...
    static $inject = ['$q', '$http', 'growl', 'CacheService', 'URLService', 'ApplicationService', 'SchemaService'];

    constructor(private $q: angular.IQService, private $http: angular.IHttpService, private growl: angular.growl.IGrowlService, private cacheSvc: CacheService, private uRLSvc : URLService,
                private applicationSvc : ApplicationService, private schemaSvc: SchemaService) {}
  ...
  if (elem.hasOwnProperty('specialization')) {
    ...
    public makeTablesAndFiguresTOCChild(child, printElement, ob, live, showRefName) {
        ...
        var refs = printElement.find('view-link[mms-pe-id="' + sysmlId + '"], view-link[data-mms-pe-id="' + sysmlId + '"]');
        var cap = '';
        var name = '';
        if (child.type === 'table') {
            ob.tableCount++;
    ...
    public isView(e) {
        if (e._appliedStereotypeIds) {
            if (e._appliedStereotypeIds.indexOf(this.schemaSvc.get('VIEW_SID', this.schema)) >= 0
                || e._appliedStereotypeIds.indexOf(this.schemaSvc.get('DOCUMENT_SID', this.schema)) >= 0) {
                return true;
            }
            const otherViewSids = this.schemaSvc.get('OTHER_VIEW_SID', this.schema);
     ...
     */
    public isDocument(e) {
        return e._appliedStereotypeIds && e._appliedStereotypeIds.indexOf(this.schemaSvc.get('DOCUMENT_SID', this.schema)) >= 0;
    };
