...
export class MentionService {
    ...
    constructor(private $interpolate, private $timeout, private cacheSvc : CacheService, private viewSvc : ViewService, private utilsSvc : UtilsService) {}
    /** Used to maintain all mention in all ckeditors **/

    

    public getFastCfListing(projectId, refId) {
        return this.cacheSvc.getLatestElements(projectId, refId).reduce((result, cacheElement) => {
            var elementType = this.viewSvc.getElementType(cacheElement);
    ...
    public createMention(editor, projectId, refId, existingMentionPlaceHolder) {
        ...
        var mention = this._createMentionDirective(editor, mentionId, projectId, refId);
        this._createNewMentionState(editor, mention, mentionPlaceHolderId, mentionId);
        MentionService._positionMentionElement(editor, mention.element, mentionPlaceHolderId);
    }
    ...
    private _createMentionDirective(editor, mentionId, projectId, refId) {
        var newScope = {
                mmsEditor: editor,
                mmsMentionValue: '',
        ...
        return this.$interpolate('<mms-mention mms-editor="mmsEditor" mms-mention-value="mmsMentionValue" mms-mention-id="mmsMentionId" mms-project-id="mmsProjectId" mms-ref-id="mmsRefId"></mms-mention>')(newScope);
    }
...
MentionService.$inject = ['$interpolate', '$timeout', 'CacheService', 'ViewService', 'UtilsService'];

veUtils.service('MentionService', MentionService);
