...
export class ProjectService {
    ...
    //ProjectService.$inject = ['$q','$http','ApplicationService','CacheService','ElementService','URLService'];
    ...
    public getProjectMounts(projectId, refId, resolve?) {
        var deferred = this.$q.defer();
        if (resolve) {
            var projPromise = this.getProject(projectId);
        }
    ...
    public getMetatypes(projectId, refId) {
        ...
        if (this.cacheSvc.exists(cacheKey)) {
            ...
            this.$http.put(url, query).then((response) => {
                ...
                var reqOb = {elementId: stereotypeKeyList, projectId: projectId, refId: refId};
