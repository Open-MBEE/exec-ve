...
import {TreeBranch, View2NodeMap} from "./TreeService.service";
...
export class ViewService {
     ...
     */
    public downgradeDocument(elementOb: angular.mms.ViewObject): angular.IPromise<angular.mms.ViewObject> {
        var deferred: angular.IDeferred<angular.mms.ViewObject> = this.$q.defer;
        ...
        this.elementSvc.updateElements([clone, asi],false)
            .then((data) => {
                ...
                return deferred.resolve(data.filter((returnOb) => {return (returnOb.id === elementOb.id)})[0]);
            }, (reason) => {
     ...
     */
    public getViewElements(reqOb: angular.mms.ElementsRequest, weight: number, update?: boolean) {
    ...
    /**
     * @ngdoc method
     * @name ViewService#handleChildViews
     * @methodOf mms.ViewService
     *
     * @description
     ...
     * @param seen
     */
    public handleChildViews(v: angular.mms.ElementObject, aggr: string, propId: string, projectId: string, refId: string, viewIdMap: View2NodeMap,
                            curItemFunc: (v: angular.mms.ElementObject, aggr: string, propId?: string ) => TreeBranch,
                            childrenFunc?: (currItem: TreeBranch, childNodes: TreeBranch[]) => void, seen?: { [key: string]: angular.mms.ElementObject }): angular.IPromise<TreeBranch | string[]> {
        var seenViews = seen;
        if (!seenViews)
            seenViews = {};
        var deferred = this.$q.defer();
        ...
        this.elementSvc.getElements({
            ...
            for (i = 0; i < childIds.length; i++) {
                ...
                if (child && this.utilsSvc.isView(child)) { //what if not found??
                    childPromises.push(this.handleChildViews(child, childAggrs[i], childPropIds[i], projectId, refId, viewIdMap, curItemFunc, childrenFunc, seenViews));
     ...
     */
    public getProjectDocuments(reqOb, weight, update?): angular.IPromise<angular.mms.ViewObject[]> {
        this.utilsSvc.normalize(reqOb);
        var deferred: angular.IDeferred<angular.mms.ViewObject[]> = this.$q.defer();
     ...
     * @returns {Promise} The promise will be resolved with array of document objects
     */
    public getProjectDocument(reqOb, weight, update?): angular.IPromise<angular.mms.ViewObject> {
    ...
    /**
     * @ngdoc method
     * @name ViewService#getPresentationElementSpec
     * @methodOf mms.ViewService
     *
     * @description
...
ViewService.$inject = ['$q', '$http', 'URLService', 'ElementService', 'UtilsService', 'CacheService', 'EventService'];
