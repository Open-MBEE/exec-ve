...
export class TableService {
    ...
    public readValues2(plot, projectId, refId, commitId) {
        ...
        if (isHeader) {
            var aheader = this._asyncReadTableHeader(aMmsEid, plot.table.header[0]);
            aheader.then((tableheader) => {
                var abody = this._asyncReadTableBody2(
                    aMmsEid,
                    plot.table.body,
                    tableheader
                );
            ...
            var abody = this._asyncReadTableBody(aMmsEid, plot.table.body);
            abody.then((tablebody) => {
    ...
    private static _getValue(datavalue) {
        if (datavalue && datavalue.type === "LiteralString") {
            ...
            datavalue &&
            (datavalue.type === "LiteralReal" || datavalue.type === "LiteralInteger")
        ) {
            return datavalue.value;
        }
    }
                    ...
                    this.elementSvc.getElement(aMmsEid, 1, false).then((refe) => {
                        ...
                        if (refe.type === "Property" || refe.type === "Port") {
                            if (refe.defaultValue) {
                                value = TableService._getValue(refe.defaultValue); //default value
                            } else {
                                value = "";
                            }
                        }
                        if (refe.type === "Slot") {
                            value = TableService._getValue(refe.value[0]); //scope.element.value
                        }
                        /* not sure what to do
...
TableService.$inject = ['$q', '$http', 'URLService', 'UtilsService', 'CacheService', 'ElementService']

veUtils.service('TableService', TableService);