...
export class ElementService {
    ...
    constructor(private $q: angular.IQService, private $http: angular.IHttpService, private uRLSvc : URLService,
     ...
     *      same object
     */
    getElement(reqOb: angular.mms.ElementsRequest, weight?, update?) {
     ...
     *      same objects
     */
    getElements(reqOb: angular.mms.ElementsRequest, weight, update?): angular.IPromise<angular.mms.ElementObject[]> {
        var deferred: angular.IDeferred<angular.mms.ElementObject[]> = this.$q.defer();
        var request = {elements: []};
        var existing: angular.mms.ElementObject[] = [];
        this.utilsSvc.normalize(reqOb);
        ...
        this.$http.put(this.uRLSvc.getPutElementsURL(reqOb), request)
            .then((response: angular.mms.ElementsResponse) => {
     ...
     * @param {angular.mms.ElementsRequest} reqOb see description of getElement.
     * @param {string} jsonKey json key that has the element array value
     * @param {integer} [weight=1] priority
     * @param {boolean} [update=false] update from server
     */
    getGenericElements(url: string, reqOb: angular.mms.ElementsRequest, jsonKey, weight, update?): angular.IPromise<angular.mms.ElementObject[]> {
        this.utilsSvc.normalize(reqOb);
        ...
        this.httpSvc.get(url,
            (data, status, headers, config) => {
                var results: angular.mms.ElementObject[] = [];
     ...
     */
    updateElements(elementObs: angular.mms.ElementObject[], returnChildViews?): angular.IPromise<angular.mms.ElementObject[]> {
        var deferred: angular.IDeferred<angular.mms.ElementObject[]> = this.$q.defer();
     ...
     *      create is successful.
     */
    createElements(reqOb): angular.IPromise<angular.mms.ElementObject[]> {
        this.utilsSvc.normalize(reqOb);
        var deferred: angular.IDeferred<angular.mms.ElementObject[]> = this.$q.defer();
        var url = this.uRLSvc.getPostElementsURL(reqOb);
     ...
     */
    isCacheOutdated(reqOb) : angular.IPromise<{status?: true, server?: angular.mms.ElementObject, cache?: angular.mms.ElementObject}> {//TODO
        var deferred = this.$q.defer();
    ...
    getElementHistory(reqOb, weight, update) {
        ...
        this.$http.get(this.uRLSvc.getElementHistoryURL(reqOb))
            .then((response: angular.mms.CommitResponse) =>{
    ...
    public getElementKey(reqOb: angular.mms.ElementsRequest, id?: string, edit?: boolean) {
        if (!id) {
    ...
    public getElementQualifiedName(reqOb: angular.mms.ElementsRequest): angular.IPromise<string> {
        let queryOb = {
            params: {
                id: reqOb.elementId
            },
        ...
        return this.search(reqOb, queryOb).then((response: angular.mms.ElementsResponse) => {
                let qualifiedName = ""
                let elements = response.elements.reverse();
