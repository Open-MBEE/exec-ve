...
export class PermissionsService {
    ...
    public lookupPermissions(
        lookups: PermissionsLookupObject[]
    ): VePromise<PermissionsLookupObject[], PermissionsLookupResponse> {
    ...
    public getProjectPermissions(
        projectId: string,
        updateCache?: boolean
    ): VePromise<PermissionsObject, PermissionsResponse> {
        ...
        return this.getPermissions(url, cacheKey, updateCache);
    }

    public getRefPermissions(
        projectId: string,
        refId: string,
        updateCache?: boolean
    ): VePromise<PermissionsObject, PermissionsResponse> {
            ...
            if (this.cacheSvc.exists(cacheKey) && !updateCache) {
                ...
                this.$http.get<PermissionsResponse>(url).then(
                    (response) => {
                        ...
                        resolve(this.cacheSvc.put<PermissionsObject>(cacheKey, perms));
                    },
        ...
        return new this.$q((resolve, reject) => {
            ...
            if (this.cacheSvc.exists(cacheKey)) {
                ...
                this.lookupPermissions(lookups).then((response) => {
                    ...
                    resolve(
                        this.cacheSvc.put<AdminObject>(cacheKey, {
                            id: projectId,
                            permission: permission == 7 ? 'admin' : permission == 6 ? 'write' : 'read',
                        })
                    );
                }, reject);
            }
        ...
        const lookups: PermissionsLookupObject[] = [
            {
                ...
                projectId: projectId,
                refId: refId,
                privilege: this.permissionsMap[type].read,
            },
            {
                type: type.toUpperCase(),
                projectId: projectId,
                refId: refId,
                privilege: this.permissionsMap[type].write,
            },
            {
                type: type.toUpperCase(),
                projectId: projectId,
                refId: refId,
                privilege: this.permissionsMap[type].admin,
            },
        ];
        return new this.$q((resolve, reject) => {
            ...
            if (this.cacheSvc.exists(cacheKey)) {
                ...
                this.lookupPermissions(lookups).then((response) => {
                    ...
                    response.forEach((lookup) => {
                        switch (lookup.privilege) {
                            case this.permissionsMap[type].read:
                                permission = permission + 4;
                                break;
                            case this.permissionsMap[type].write:
                                permission = permission + 2;
                                break;
                            case this.permissionsMap[type].admin:
                                permission = permission + 1;
                                break;
                        }
                    });
    ...
    public initializePermissions(
        ...
        return new this.$q((resolve, reject) => {
            const promises: VePromise<void, PermissionsLookupResponse>[] = [];
            promises.push(
                this.getOrgPermission(orgId).then((result) => {
            ...
            if (projectId) {
                promises.push(
                    this.getProjectPermission(projectId).then((result) => {
            ...
            if (refId) {
                promises.push(
                    this.getRefPermission(projectId, refId).then((result) => {
    ...
    public hasBranchEditPermission = (projectId: string, refId: string): boolean => {
        return (
            this.permissions.ref[projectId + '/' + refId] == 'edit' ||
            this.permissions.ref[projectId + '/' + refId] == 'admin'
        );
    };
