...
function CacheService(_) {
    ...
    var put = function(key, value, merge) {
        ...
        if (val && m && angular.isObject(value)) {
            _.mergeWith(val, value, function(objValue, srcValue, key) {
                if ((key === '_contents' || key === 'specification') && srcValue && srcValue.type === 'Expression') {
                    return srcValue;
                }
                if (angular.isArray(objValue) && angular.isArray(srcValue) && srcValue.length < objValue.length) {
                    objValue.length = 0;
                    Array.prototype.push.apply(objValue, srcValue);
                    return objValue;
                }
                if (key === '_displayedElementIds' && srcValue) {
                    return srcValue;
                }
                // If the custom function returns 'undefined', _.mergeWith will handle the merge as usual.
                return undefined;
            });
        } else {
