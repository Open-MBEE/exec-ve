            ...
            <list ng-if="$ctrl.users.length > 0">
                ...
                <div class="user-info" ng-repeat="user in $ctrl.users" key="user-info-{{user.username}}">
                    <user-list-item class-name="user-name"
                        ...
                        />
                    <div class="controls-container" ng-if="">
                        <span uib-tooltip="Edit" tooltip-placement="top" target="edit-user-{{user.username}}">
                            <i id="edit-user-{{user.username}}"
                                ng-click={() => $ctrl.handleEditToggle(user)}
                                class="fas fa-user-edit add-btn"/>
                        </span>
                        <span uib-tooltip="Delete" tooltip-placement="top" target="delete-{{user.username}}">
