...
let VeNavComponent: angular.ve.ComponentOptions = {
    ...
    <div class="block">
        ...
        <div class="collapse navbar-collapse" uib-collapse="$ctrl.isNavCollapsed" id="global-navigation">
            <form id="global-search" ng-submit="$ctrl.search(searchText)" class="navbar-form navbar-left" role="search">
                ...
                <li class="dropdown">
                    <span class="dropdown ve-dark-dropdown-nav fixed-content-dropdown user-profile" uib-dropdown on-toggle="$ctrl.toggled(open)">
                        <a href id="login-dropdown" class="dropdown-toggle" uib-dropdown-toggle>
                            <div ng-if="$ctrl.user.firstName !== undefined">{{ $ctrl.user.firstName | limitTo:1 | uppercase }}{{ user.lastName | limitTo:1 | uppercase }}</div>
                            <div ng-if="$ctrl.user.firstName === undefined">{{ $ctrl.user | limitTo:2 | uppercase }}</div>
                        </a>
...
e
        ...
        search(searchText) {
            if (this.$state.includes('main.project.ref.document.order')) {
                ...
                if (this.$state.params.search === searchText)
                    return;
                this.searchClass = "fa fa-spin fa-spinner";
                this.$state.go(this.$state.current.name, {search: searchText});
            }
        };
...
mmsApp.component(VeNavComponent.selector,VeNavComponent);