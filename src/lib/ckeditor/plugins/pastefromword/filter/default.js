    ...
    q.lists = {
        ...
        setListStart: function (c) {
            ...
            switch (c.attributes['cke-list-style-type']) {
                ...
                case 'upper-alpha':
                    c.attributes.start =
                        g.getSubsectionSymbol(b[d]).replace(/\W/g, '').toLowerCase().charCodeAt(0) - 96 - d;
                    break;
                case 'decimal':
        ...
        numbering: {
            ...
            getStyle: function (c) {
                ...
                b ||
                    ((b = 'decimal'), c.match(/[a-z]/) && (b = 'lower-alpha'), c.match(/[A-Z]/) && (b = 'upper-alpha'));
                return b;
            },
        },
            ...
            for (c = 0; c < k.length; c++) {
                ...
                for (f = 0; f < a.length; f++) {
                    ...
                    for (e = d.attributes['cke-list-level']; e > h.length; ) {
                        ...
                        0 < x.length
                            ? x[x.length - 1].add(v)
                            : ((x = new CKEDITOR.htmlParser.element('li', { style: 'list-style-type:none' })),
                              x.add(v),
                              m.add(x));
                        h.push(v);
                        u.push(v);
            ...
            if (this.isShifted(c)) {
                var b = CKEDITOR.tools.array.filter(c.children, function (b) {
                    ...
                    d = CKEDITOR.tools.array.reduce(
                        ...
                        function (b, c) {
                            return (
                                c.children && 1 == c.children.length && p.isShifted(c.children[0]) ? [c] : c.children
                            ).concat(b);
                        },
                        []
                    );
            ...
            return 'li' !== c.name
                ...
                : 0 ===
                      CKEDITOR.tools.array.filter(c.children, function (b) {
                          return b.name &&
                              ('ul' == b.name || 'ol' == b.name || ('p' == b.name && 0 === b.children.length))
                              ? !1
                              : !0;
                      }).length;
        },
