...
export class MmsHtmlDiffController implements angular.IComponentController {
    ...
    diffContainer
        ...
        .each((index, element) => {
            const img$ = $(element);
            const imgPatcherClass = img$.hasClass('patcher-insert') ? 'patcher-insert' : img$.hasClass('patcher-delete') ? 'patcher-delete' : null;
            if (imgPatcherClass) {
     ...
     private _formatRowDiff = (diffContainer: JQuery<HTMLElement>) => {
        ...
        .each((index, element) => {
            ...
            const trPatcherClass: string = tr$.hasClass('patcher-insert') ? 'patcher-insert' : tr$.hasClass('patcher-delete') ? 'patcher-delete' : '';
            if (trPatcherClass !== '') {
