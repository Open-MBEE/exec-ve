...
veCore.directive('mmsD3GroupedHorizontalBarPlot', ['PlotService','$window', mmsD3GroupedHorizontalBarPlot]);
function mmsD3GroupedHorizontalBarPlot(PlotService, $window) {
    ...
    var mmsChartLink = function(scope, element, attrs, mmsViewCtrl) {
     ...
     var plotConfig = PlotService.plotConfig(scope.plot.config.options, defaultPlotConfig);
      var achartdata;
      ...
      function createFilters(data){
            ...
            var filterLegendsDiv = graphFilter.append('div')
              ...
              filterLegendsDiv
                ...
                .enter()  
                .append("div")
                .attr("class", function(d,i){return "legentFilter "+ data.id + " " + PlotService.toValidId(d);} )
                ...
                .each(function (d, i) {
                      ...
                      d3.select(this).append("input")
                        ...
                        .attr("checked",function(d,i){
                          if (data.legendsFilter[PlotService.toValidId(d)] === true)
                            return true;
                          else 
                            return null;
                        })
              ...
              filterColoumnsDiv
                ...
                .enter()  
                .append("div")
                .attr("class", function(d,i){return "columnfilter "+ data.id + " " + PlotService.toValidId(d);} )
                .on('mouseover', function (d, i){
                      mouseover('.'+data.id+'.' + PlotService.toValidId(d));
                })
                .on('mouseout', function (d, i){
                ...
                .each(function (d, i) {
                    ...
                    d3.select(this).append("input")
                      .attr("type", "checkbox")
                      .attr("checked", function(d,i){
                          if (data.labelsFilter[PlotService.toValidId(d)] === true)
                            return true;
                          else 
                            return null;
                      })
      ...
      function createGroupedHorizontalBarChart(data)
      {
        ...
        var filteredDataValues: any[] = [];
        var filteredDataSysmlids: any[] = [];
        var filteredDataColors: any[] = [];
        var filteredDataLegends: any[] = [];//table row headers
        var filteredDataLabels: any[] = [];//table column headers
        var datalegendsfilter: any[] = data.legendsFilter;
        var datalabelsfilter: any[] = data.labelsFilter;
        var counter = -1;
        ...
        bar.append("rect")
            ...
            .attr("class", function(d,i){ 
              return "ghbbar "+ data.id + " " + filteredDataLegends[i] + " " + PlotService.toValidId(filteredDataLabels[Math.floor(i/numRows)]);
              })
            .attr("width", x)
        ...
        legend.append('rect')
            .attr('class', function(d,i){ return 'legendRect ' + data.id+ " " + PlotService.toValidId(d);})
      ...
      scope.render = function() {
        PlotService.readValues(scope.plot, projectId, refId, commitId)
       .then( function(value){
        ...
        var cellIds: number[] = []; //0, 1, 2, 3
        var legends: any[] = [];
        var legendsFilter: boolean[] = [];
        var rowvalue: number[];
        var cellId: number[];
        var cid: number = 0;
        ...
        var labelsFilter: { [key: string]: boolean } = {};
