...
export class TreeService {
    ...
    constructor(private $timeout: angular.ITimeoutService, private applicationSvc: ApplicationService, private eventSvc: EventService) {
    }

    getApi(config?: TreeConfig): TreeApi {
        if (config && !this.treeApi)
            this.treeApi = new TreeApi(this.$timeout, this.eventSvc, config, false);
        return this.treeApi
    }

    getPeApi (config: TreeConfig): TreeApi {
        if (!this.peTreeApis[config.id])
            this.peTreeApis[config.id] = new TreeApi(this.$timeout, this.eventSvc, config, true);
    ...
    // Function to refresh pe trees when new item added, deleted or reordered
    ...
    refreshPeTree(treeId: string, cb?: () => void) {
        if (this.peTreeApis[treeId]) {
            ...
            this.getPeTreeData(this.treeApi.getRows()[0].branch, api.treeConfig.types, api.treeData)
            api.refresh().then(() => {
                if (cb)
                    cb();
            })
    ...
    getPeTreeData(branch: TreeBranch, types: string[], peTreeData: TreeBranch[]) {
