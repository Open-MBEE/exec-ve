...
let ViewTableComponent: PresentationComponentOptions = {
    ...
    <span class = "ve-show-filter" ng-show="$ctrl.showFilter">
        <form style="display: inline" class="ve-filter-table-form"><input type="text" size="75" placeholder="Filter table" ng-model="searchTerm" ng-model-options="$ctrl.ngModelOptions"></form>
...
`,
    ...
    controller: class ViewTableController implements angular.IComponentController {
        ...
        constructor(private $scope: angular.IScope, private $compile: angular.ICompileService,
                    ...
                    private viewHtmlSvc: ViewHtmlService, private extUtilSvc: ExtUtilService, private eventSvc: EventService) {}

        $onInit = () => {

            if (!this.viewData.showIfEmpty && this.viewData.body.length === 0) {
                return;
            }


            this.table = this.viewData;
            ...
            let html = this.viewHtmlSvc.makeHtmlTable(this.table, true, true, this.viewPe);
            $(this.$element).append('<div class="table-wrapper">' + html + '</div>')

        }

        /** Filter Utilities **/
        public searchTerm: Function = (searchTerm=null) => {
            if (searchTerm === null) {
                return this._searchTerm
            }
            this.eventSvc.$broadcast('update-search',{newInputVal: searchTerm, oldInputVal: this._searchTerm});
            return this._searchTerm = searchTerm;
        }
        ...
        public scroll = () => {
            ...
            if (this._fixedHeadersElem && this._fixedColumnsElem) {
                this.$element.find('caption').css('transform', function (this: HTMLElement, index: number, value: string) {
        ...
        public makeFixedHeader = () => {
                if (!this.fixedHeaders) {
                    ...
                    this.$window.localStorage.setItem('ve-table-header-' + this.viewPe.id, 'false');
                    return;
                }
                ...
                this.$window.localStorage.setItem('ve-table-header-' + this.viewPe.id, 'true');
        }
        ...
        public makeFixedColumn = () => {
            if (!this.fixedColumns) {
               this.$element.find('.table-wrapper').removeClass('table-fix-column').css('width', '');
                ...
                this.$window.localStorage.setItem('ve-table-column-' + this.viewPe.id, 'false');
                return;
            }
            ...
            this.$window.localStorage.setItem('ve-table-column-' + this.viewPe.id, this.numFixedColumns.toString());
        };
        ...
        $postLink() {
            ...
            this.$compile(thead)(this.$scope);
            var searchbar = $(this.$element).children('div')[0];
            this.$compile(searchbar)(this.$scope);
            this.$compile($(this.$element).find('caption'))(this.$scope);
           ...
           this.$timeout(() => {
               ...
               this.nextIndex = first + 300;
                if (this.nextIndex < this.lastIndex)
                    ...
                    if (this.$window.localStorage.getItem('ve-table-header-' + this.viewPe.id) == 'true') {
                        this.fixedHeaders = true;
                        this.makeFixedHeader();
                    }
                    var columnFix = this.$window.localStorage.getItem('ve-table-column-' + this.viewPe.id);
...
veExt.component(ViewTableComponent.selector,ViewTableComponent);