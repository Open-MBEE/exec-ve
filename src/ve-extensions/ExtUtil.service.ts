...
export class ExtUtilService {
     ...
     * @ngdoc function
     * @name Utils#save
     * @methodOf veExt.ExtUtilsService
     *
     * @description
     * save edited element
     *
     ...
     */
    public save(edit: ElementObject, editorApi: VeEditorApi, ctrl: {element: ElementObject, values?: any[]}, continueEdit: boolean) {
        var deferred = this.$q.defer();
        if (editorApi && editorApi.save) {
            editorApi.save();
        }
        this.elementSvc.updateElement(edit, false,true)
        .then((element) => {
            ...
            if (reason.status === 409) {
                ...
                instance.result.then((data) => {
                    ...
                    if (choice === 'ok') {
                        ...
                        this.save(edit, editorApi, ctrl, continueEdit).then((resolved) => {
     ...
     */
    public revertEdits(editValues: any[], editOb: ElementObject, editorApi?: VeEditorApi) {
     ...
     */
    public isEnumeration(elementOb: ElementObject): angular.IPromise<PropertySpec> {
        var deferred: angular.IDeferred<PropertySpec> = this.$q.defer();
    ...
    public getPropertySpec(elementOb: ElementObject): angular.IPromise<PropertySpec> {
        var deferred: angular.IDeferred<PropertySpec> = this.$q.defer();
    ...
    */
    public startEdit(ctrl: ExtensionController,
                     mmsViewCtrl, domElement: JQuery<HTMLElement>, template: string | Injectable<(...args: any[]) => string>, doNotScroll): void {
        if (mmsViewCtrl.isEditable() && !ctrl.isEditing && ctrl.element && ctrl.commitId === 'latest' && this.permissionsSvc.hasProjectIdBranchIdEditPermission(ctrl.element._projectId, ctrl.element._refId)) {
     ...
     */
    public saveAction(ctrl: ExtensionController, domElement: JQuery, continueEdit) {
    ...
    * @param {object} domElement dom of the directive, jquery wrapped
    */
    public cancelAction(ctrl: ExtensionController, recompile:(preview?) => void, domElement: JQuery<HTMLElement>) {
        if (ctrl.elementSaving) {
            this.growl.info('Please Wait...');
            return;
        }
    ...
    public deleteEditModal(deleteOb) {
        return this.$uibModal.open({
            ...
            resolve: {
                ...
                ok: () => {
                    this.clearAutosave(deleteOb.element._projectId + deleteOb.element._refId + deleteOb.element.id, deleteOb.type);
                    return true;
                }
            }
        });
    ...
    public deleteAction(ctrl: ExtensionController, bbApi: ButtonBarApi, section: ViewObject) {
        if (ctrl.elementSaving) {
            this.growl.info('Please Wait...');
            return;
        }
        ...
        instance.result.then(() => {
            var viewOrSec = section ? section : ctrl.view;
            var reqOb = {elementId: viewOrSec.id, projectId: viewOrSec._projectId, refId: viewOrSec._refId, commitId: 'latest'};
            this.viewSvc.removeElementFromViewOrSection(reqOb, ctrl.instanceVal)
            .then((data) => {
                if (this.viewSvc.isSection(ctrl.instanceSpec) || this.viewSvc.isTable(ctrl.instanceSpec) || this.viewSvc.isFigure(ctrl.instanceSpec) || this.viewSvc.isEquation(ctrl.instanceSpec)) {
