    ...
    controller: class VeMenuController implements angular.IComponentController {
        ...
        updateBranch(branch) {
            if (branch.status != 'creating') {
                this.$state.go(this.$globalState.$current.name, {projectId: this.mmsProject.id, refId: branch.id, search: undefined}, {reload: true});
            }
        };

        updateTag(tag) {
            if (tag.status != 'creating') {
                this.$state.go(this.$globalState.$current.name, {projectId: this.mmsProject.id, refId: tag.id, search: undefined}, {reload: true});
            }
        };
