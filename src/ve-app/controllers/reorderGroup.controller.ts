...
veApp.controller('ReorderGroupCtrl', ['$scope', '$stateParams', '$state', 'growl', 'ElementService', 'CacheService', 'TreeService', 'projectOb', 'refOb', 'groupObs', 'documentObs',
...
function ($scope, $stateParams, $state, $window, growl, ElementService, CacheService, TreeService, projectOb, refOb, groupObs, documentObs) {
    ...
    function navigateAway(reload) {
        var curBranch = TreeService.getApi().getSelectedBranch();
        if (curBranch) {
