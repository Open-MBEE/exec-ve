...
`,
    controller: class MainController implements angular.IComponentController {
        ...
        constructor(private $scope: angular.IScope, private $timeout: angular.ITimeoutService, private $location: angular.ILocationService,
                    private $window: angular.IWindowService, private $uibModal: VeModalService,
        ...
        $onInit() {
            ...
            this.$transitions.onStart({}, (trans) => {
                ...
                if (from.split('.').length >= to.split('.').length) {
                    ...
                    //console.log(
                    trans.router.viewService._pluginapi._activeViewConfigs().filter((vc) => {
            ...
            this.$transitions.onError({}, (trans: Transition) => {
                ...
                if (!error || error.status === 401 ||
                    ...
                    trans.abort();
                    this.rootScopeSvc.veRedirect({toState: trans.to(), toParams: trans.params()});
                    this.$state.target('main.login', {next: trans.$to().url});
                }
                if (this.veConfig.enableDebug) {
                    this.growl.warning('Error: ' + trans.error().message,{ttl: 1000});
                }
            ...
            this.$transitions.onSuccess({}, (trans: Transition) => {
                ...
                if (this.rootScopeSvc.veRedirect() && this.$uiRouterGlobals.$current.name === this.rootScopeSvc.veRedirect().toState.name) {
                ...
                } else if (this.$state.includes('project') && !(this.$state.includes('main.project.refs'))) {
                    this.rootScopeSvc.veHidePanes(true);
                    this.rootScopeSvc.veShowManageRefs(true);
                    this.eventSvc.$broadcast('fromParamChange', trans.params('from'));
                }
                if (this.$uiRouterGlobals.$current.name === 'main.project.ref.portal') {
