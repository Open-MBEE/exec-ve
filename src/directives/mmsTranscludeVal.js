...
angular.module('mms.directives').directive('mmsTranscludeVal',['ElementService','UtilsService','UxService','Utils','URLService','$http','_','$log','$compile','$templateCache','growl',mmsTranscludeVal]);
...
function mmsTranscludeVal(ElementService,UtilsService,UxService,Utils,URLService,$http,_,$log,$compile,$templateCache,growl){
    ...
    var mmsTranscludeValLink=function(scope,element,attrs,controllers){
        ...
        var recompile=function(){
            ...
            for(var i=0; i<scope.values.length; i++){
                if(scope.values[i].type==='LiteralString'){
                    ...
                    if(scope.values[i].type==="Expression"){
                        isExpression=true;
                        break;
                    }else{
                        break;
            ...
            // Checks if element is an expression
            if(isExpression){
                // Added in response statement when value.type is
                // Expression
                $http.get(URLService.getElementURL(scope.mmsEid,scope.ws,scope.version)+'?evaluate').success(function(data,status,headers,config){
                    if(isExpression){
        ...
        var recompileEdit=function(){
            ...
            if(scope.editValues.length===0||Object.keys(scope.editValues[0]).length<2){
                ...
                if(areStrings){
                    ...
                    if(UtilsService.isRestrictedValue(scope.editValues)){
        ...
        scope.addValueTypes={
            string:'LiteralString',boolean:'LiteralBoolean',integer:'LiteralInteger',real:'LiteralReal'
        };
