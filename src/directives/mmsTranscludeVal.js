    ...
    var mmsTranscludeValLink = function (scope, element, attrs, controllers) {
        ...
        var recompile = function () {
            ...
            for (var i = 0; i < scope.values.length; i++) {
                ...
                else if (scope.values[i].type === "Property" && scope.values[i].value.type === "Expression") {
                    isProperty = true;
                    break;
                }
                else if (scope.values[i].type === "Constraint") {
                    isConstraint = true;
                    break;
                }
                else {
            ...
            if (isExpression || isConstraint || isProperty) {
                ...
                $http.get(URLService.getElementURL(scope.mmsEid, scope.ws, scope.version) + '?evaulate')
                    .success(function (data, status, headers, config) {
                        if (isExpression) {
                            element.html(data.elements[0].specialization.evaluation);
                        } else if (isConstraint) {
                            element.html(data.elements[0].specialization.specification.operand);
                        } else {
                            element.html(data.elements[0].specialization.value.operand);
                        }
                        // TODO : change evaluation to evaluationResult
