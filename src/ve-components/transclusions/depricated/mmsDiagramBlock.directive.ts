...
veComponents.directive('mmsDiagramBlock', ['go', 'growl', 'ElementService', mmsDiagramBlock]);
      ...
      .then(function(data) {
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          if (elem.type === 'Dependency') {
            ...
            var edge: {from: string, to: string} = { from: elem._sourceIds[0], to: elem._targetIds[0]};
