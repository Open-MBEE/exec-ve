...
class ToolsPaneController implements angular.IComponentController {
    ...
    static $inject = ['$compile', '$scope', '$element', '$uibModal', '$q', '$timeout', 'hotkeys', 'growl',
        'ElementService', 'ProjectService', 'ComponentService', 'PermissionsService', 'RootScopeService', 'EventService',
        'AutosaveService', 'ToolbarService', 'SpecService', 'ReorderService', 'ExtensionService']
    ...
    constructor(private $compile: angular.ICompileService, private $scope, private $element: JQuery,
                ...
                private elementSvc: ElementService, private projectSvc: ProjectService, private componentSvc: ComponentService,
    ...
    $onInit() {
        ...
        this.subs.push(this.eventSvc.$on('spec-editor-saveall', () => {
            ...
            this.elementSvc.updateElements(Object.values(this.autosaveSvc.getAll()))
                .then((responses) => {
                    ...
                    var elementToSelect = (<ElementObject>Object.values(this.autosaveSvc.getAll())[0]);
        ...
        this.subs.push(this.eventSvc.$on('spec-editor-cancel', () => {
            let go = () => {
                ...
                if (this.autosaveSvc.openEdits() > 0) {
                    var next = Object.keys(this.autosaveSvc.getAll())[0];
                    var id = next.split('|');
    ...
    $postLink() {
        if (!this.currentTool) {
            this.currentTool = '';
            const inspect: IToolBarButton = this.toolbarSvc.getToolbarButton(this.defaultTool)
