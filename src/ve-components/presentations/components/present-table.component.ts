...
import {IPresentation, Presentation, PresentationComponentOptions, PresentationService} from "@ve-components/presentations";
import {EventService, ImageService} from "@ve-utils/services";
import {TableCell, TableConfig, ViewHtmlService} from "@ve-components/presentations";
...
class PresentTableController extends Presentation implements IPresentation {
    ...
    /** Column Filter **/
    private _columnsInputSubs: { [columnId: string]: {sub: Rx.IDisposable, cell: TableCell} } = {};
    ...
    constructor($element: JQuery<HTMLElement>, $scope: angular.IScope,
                ...
                presentationSvc: PresentationService,  componentSvc: ComponentService, eventSvc: EventService,
                imageSvc: ImageService, buttonBarSvc: ButtonBarService, private $document: angular.IDocumentService,
    ...
    private _getCellValueForSorting = (row: HTMLElement, columnIndex: number) => {
        ...
        if (containerDivContent.length === 0) {
            ...
            const cf = 'mms-cf';
            const contentTag = containerDivContent.prop('tagName').toLowerCase();
    ...
    makeCsv = () => {
        const csvString = this.$element.find('.table-wrapper').children('table').table2CSV({delivery:'value'});
    ...
    private compileTable() {
        this.$timeout(() => {
            ...
            if (this.nextIndex < this.lastIndex)
                ...
                const columnFix = this.$window.localStorage.getItem('ve-table-column-' + this.element.id);
