...
describe('UtilsService', function() {
    ...
    describe('Method cleanElement ', function() {
        ...
        it('for every value of elem.specialization.contents if there\'s a valueExpression in any of its children delete it', inject(function() {
            // This will behave the same as the test above, except it should be removing the "values" of an element
            // which is an array of ValueSpecifications
            // :TODO what does this object look like?
            var dirtyElement4 = {author:'muschek', sysmlid:38742, name:'dirtyElement4', owner:'otherElement',
                specialization: {type:'Property', isDerived:false, isSlot:false, propertyType:'anotherElementID',
                    value:[{valueExpression:'valueExpression', type:'ValueWithSpec', specialization:{type:'Unknown'}}]}};
        }));
        xit('for every value of elem.specialization.instanceSpecificationSpecification if there\'s a valueExpression in any of its children delete it', inject(function() {

        }));
        xit('If elem.specialization.type is view and has both keys contents and contains delete contains', inject(function() {

        }));
        xit('If elem.specialization.type is view and has a Array of (key) displayed elements whose length is less then 5000 delete it', inject(function() {

        }));
        xit('If elem.specialization.type is view and has a Array of (key) displayed elements whose length is greater then 5000 convert the array to JSON', inject(function() {

        }));
        xit('should delete any nonEditable keys from the object', inject(function() {
