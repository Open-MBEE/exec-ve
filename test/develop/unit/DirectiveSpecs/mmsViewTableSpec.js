...
describe('Directive: mmsViewTable', function () {
    ...
    function replaceDom(trs) {
        ...
        innerTableRows
            ...
            .find('mms-cf')
            .replaceWith(
                '<mms-cf mms-cf-type="name"><span>value1</span></mms-cf>'
            )
        innerTableRows
            .first()
            .children('td:nth-child(2)')
            .find('mms-cf')
            .replaceWith(
                '<mms-cf mms-cf-type="val"><span>234111070</span></mms-cf>'
            )
        $(innerTableRows[1])
            .children('td:nth-child(1)')
            .find('mms-cf')
            .replaceWith(
                '<mms-cf mms-cf-type="name"><span>value2</span></mms-cf>'
            )
        $(innerTableRows[1])
            .children('td:nth-child(2)')
            .find('mms-cf')
            .replaceWith(
                '<mms-cf mms-cf-type="val"><span class="no-print">(no value)</span></mms-cf>'
            )
        ...
        $(trs[1])
            ...
            .find('mms-cf')
            .replaceWith(
                '<mms-cf mms-cf-type="doc"><span>adfew dfvdfdsf</span></mms-cf>'
            )
    }
    ...
    beforeEach(function () {
        ...
        element = angular.element(
            '<mms-view-table data-mms-table="presentationElem"></mms-view-table>'
        )
    ...
    it('store rows order as an attribute so that it can reset the sort order', function () {
        this.trs.each(function (index) {
            expect(
                Number(
                    $(this).attr(
                        self.mmsViewTableController._rowSortOrderAttrName
                    )
                )
            ).toEqual(index)
        })
    ...
    it('can do Full Table Filter ', function () {
        /** Filter by 'g' **/
        var fullTableFilterInput = element
            .find('form')
            .children('input')
            .first()
    ...
    it('use the numerical sort when all cells value is a valid number', function () {
        ...
        var element = angular.element(
            '<mms-view-table data-mms-table="tableData"></mms-view-table>'
        )
        ...
        expect(
            self.mmsViewTableController.__areAllCellValidNumber(
                trs.toArray(),
                0
            )
        ).toBeTruthy()
