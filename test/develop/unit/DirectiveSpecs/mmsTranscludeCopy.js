describe('mmsTranscludeCopy directive', function () {
    ...
    it('mmsTranscludeCom should translude a comment within the element', inject(function () {
        ...
        element = angular.element('<mms-transclude-com data-mms-element-id="{{view.sysmlId}}"></mms-transclude-com>');
        $compile(element)(scope);
        scope.$digest();

        expect(element.html()).toContain("Putting some text in here!!!");
        expect(element.html()).toContain("- mmsadmin");
        console.log("old : " + element.html());
    }));
});